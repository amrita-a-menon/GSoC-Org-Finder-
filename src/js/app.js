...
// HELPERS
// ══════════════════════════════════════════════
function yCls(y){ if(y>=8)return'veteran'; if(y>=4)return'experienced'; return'newcomer'; }
function yLbl(y){ if(y>=8)return'🏆 Veteran'; if(y>=4)return'⭐ Experienced'; return'🌱 Newcomer'; }
function yBdg(y){ if(y>=8)return'bv'; if(y>=4)return'be'; return'bn'; }
function cLbl(c){ if(c==='hot')return'🔥 High'; if(c==='moderate')return'🟡 Moderate'; return'😎 Low'; }
function cBdg(c){ if(c==='hot')return'bh'; if(c==='moderate')return'bm'; return'bc'; }
function aLbl(a){ if(a==='active')return'⚡ Active'; if(a==='moderate')return'📊 Moderate'; if(a==='low')return'💤 Low'; return'○ —'; }
function aBdg(a){ if(a==='active')return'bac'; if(a==='moderate')return'bam'; if(a==='low')return'bal'; return'bna'; }
...
function renderCompareTable(){
  ...
  const thead = `<tr><th>Metric</th>${thCells}</tr>`;  let tbody='';
    ...
    return`<div class="org-card${inCompare?' in-compare':''}${isFocused?' focused':''}"
      ...
      <div class="card-header-row">
        ...
        <div class="org-logo-info">
          ...
          const repoIcon = (UMBRELLA_ORGS.has(o.name) || !repoPath.includes('/'))
...
function openModal(idx){
  ...
  for(let y=o.firstYear;y<=2026;y++){
    ...
    tl += `<span style="margin-right:10px;color:${color};font-weight:${weight}">`;
  }
